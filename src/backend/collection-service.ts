  ...
  public async list(query: string = '', limit?: number): Promise<CollectionListResult> {
    ...
    const selectSql = `
      SELECT
        ...
        file_identification_state.verified_at AS identificationVerifiedAt,
        ...
        FROM (
          SELECT
            ...
            COALESCE(
              ...
              (
                SELECT audio_assets.duration_seconds
                FROM audio_assets
                WHERE audio_assets.recording_id = recordings.id
                  AND audio_assets.duration_seconds IS NOT NULL
                ORDER BY audio_assets.confidence DESC, audio_assets.updated_at DESC, audio_assets.audio_hash
                LIMIT 1
              )
            ) AS duration_seconds
          FROM recordings
    ...
    await this.pool.query(
      `
        INSERT INTO file_identification_state(
          ...
          tag_artist, tag_title, tag_version, chosen_claim_id,
          identify_version, explanation_json, review_data, verified_at, error_message, processed_at
        ) VALUES ($1, $2, $3, NULL, NULL, 'error', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, $4, NULL, NULL, NULL, $5, now())
        ON CONFLICT(filename) DO UPDATE SET
          filesize = excluded.filesize,
          mtime_ms = excluded.mtime_ms,
          status = 'error',
        ...
        if (selectedCandidate) {
          await client.query(
            `
              ...
              WHERE filename = $1 AND id = $2
            `,
            [filename, selectedCandidate.id, recordingId]
          )
        }
