...
import { IdentifyRecordCandidates, type IdentifyViewCandidate } from './IdentifyRecordCandidates'
...
export function IdentifyEvidenceTable({
  ...
  const { item, error, isPending, refetch, searchDraft, setSearchDraft, selectedCandidateId, setSelectedCandidateId, nextItemId, reviewData, inferredReferences, groups } = useIdentifyReviewData(itemId, scope, query, filter)

  const canIdentify = Boolean(item && item.identificationStatus !== 'processing')
  const errorMessage = error instanceof Error ? error.message : error ? 'Failed to load identification.' : null
  ...
  const assignAndNext = (): void => {
    ...
    void actions.run({
      key: 'assign-next',
      action: async () => {
        await api.collection.reviewIdentification({ filename: item.filename, action: 'accept', candidateId: selectedCandidateId })
        goToNext()
      },
  ...
  return (
    <ViewSection
      ...
      aside={<ActionButton size="xs" onClick={onBack}>Back</ActionButton>}
    >
        ...
        <div className="space-y-2">
          <div className="flex flex-wrap items-center gap-2 text-xs">
            ...
            <Pill tone={item.identification?.verifiedAt ? 'primary' : 'muted'}>
              {item.identification?.verifiedAt ? `Verified ${new Date(item.identification.verifiedAt).toLocaleString()}` : 'Unverified'}
            </Pill>
          </div>
          ...
          <div className="rounded border border-zinc-800 bg-zinc-950/40 px-2 py-2">
            <audio ref={audioRef} className="w-full" controls preload="metadata" src={localFileUrl('', item.filename)}>
              <track kind="captions" />
            </audio>
          </div>
          {!reviewData?.recordCandidates.length ? <Notice tone="warning">No stored review snapshot yet. Reidentify to build record candidates.</Notice> : null}
          {inferredReferences.length || groups.length ? (
            <IdentifyRecordCandidates
              candidates={groups}
              inferredReferences={inferredReferences}
              onPlayLocal={(_reference: IdentifyReference) => playLocal()}
              onPlayExternal={(url) => window.open(url, '_blank', 'noopener,noreferrer')}
              onToggleCandidate={(candidateId) => setSelectedCandidateId((current) => current === candidateId ? null : candidateId)}
            />
          ) : null}
