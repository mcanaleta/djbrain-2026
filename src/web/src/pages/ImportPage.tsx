...
import { ActionButton, DataTable, LabeledInput, Notice, Pill, ViewSection, type DataTableColumn } from '../components/view'
...
export default function ImportPage(): React.JSX.Element {
  ...
  useEffect(() => {
    if (!groupByTrack) return
    const filenames = rows
      .filter((row) => row.prep === 'pending' && !autoQueuedRef.current.has(row.filename))
      .map((row) => row.filename)
    if (filenames.length === 0) return
    filenames.forEach((filename) => autoQueuedRef.current.add(filename))
    api.collection.queueImportProcessing(filenames).then((result) => {
      ...
      for (const filename of filenames) autoQueuedRef.current.delete(filename)
    })
  }, [groupByTrack, rows])
  ...
  return (
    <div className="space-y-4">
      ...
      <ViewSection
        ...
        subtitle={
          groupByTrack
            ? 'Grouped by resolved Discogs track so duplicates collapse into one review row. Pending files warm in background.'
      ...
      >
        {groupByTrack ? (
          <DataTable
            ...
            onRowClick={(row) => navigate(`/import/review?filename=${encodeURIComponent(row.bestFile.filename)}`)}
