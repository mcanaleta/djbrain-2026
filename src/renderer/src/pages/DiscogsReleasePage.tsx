...
export default function DiscogsReleasePage(): React.JSX.Element {
  ...
  const { data: release, isLoading, error } = useQuery({
    ...
    queryFn: () => window.api.onlineSearch.getDiscogsEntity('release', id),
    enabled: Number.isInteger(id) && id > 0
  })

  const { data: collectionItems = [] } = useQuery({
    queryKey: ['collection'],
    queryFn: async () => (await window.api.collection.list()).items,
    staleTime: 60_000
  })

  const { data: downloadItems = [] } = useQuery({
    queryKey: ['collection-downloads'],
    queryFn: async () => (await window.api.collection.listDownloads()).items,
    staleTime: 60_000
  })
