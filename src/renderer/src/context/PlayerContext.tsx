...
export function PlayerProvider({ children }: { children: React.ReactNode }): React.JSX.Element {
  ...
  const log = (...args: unknown[]): void => console.log('[player]', ...args)

  function clearSeekTimeout(): void {
  ...
  function safePlay(audio: HTMLAudioElement, reason: string): void {
    log(
  ...
  function safePause(audio: HTMLAudioElement, reason: string): void {
  ...
  function loadTrack(audio: HTMLAudioElement, nextTrack: PlayerTrack, reason: string): void {
    clearSeekTimeout()
    clearStallRecovery()
    pendingPlayRef.current = null
